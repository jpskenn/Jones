...
試作を重ねる中でModキーの配置は何度か変更していますが、Jonesの特徴である2行目と3行目にずれのないレイアウトは、最初の試作から変わることなく続いています。

振り返ってみて思うことですが、もしこのレイアウトが使いにくいものだったなら、1年以上も使い続けるでしょうか？  
打鍵感が好きで使っていたRealforce R2は、Jonesを使い始めてから出番がなくなり、押し入れに入れられたままになっています。  
この状況から考えると、少なくとも、RalforceよりもJonesのほうが使いやすいと感じる何かがあるのでしょう。

### キーボードのセッティング

だいたいの感じは、[Remapで共有しているキーマップ](https://remap-keys.app/catalog/z8HynboRCOQfPmwXbzp6/keymap?id=NXDzVJVuxeW8HdR5joy8)を見てもらうと伝わるでしょうか？

- レイアウト  
  使い始めから1年近くはNarrowレイアウトで使ってきましたが、現在はWideレイアウトで使っています。  
  肩への負担を少し減らし、最下行中央のキーに機能を割り当てて親指を活用しようとしています。  

- キーマップ  
  英語配列を基本としたキーマップです。  
  日本語で長音（伸ばし棒）`ー`を入力することが多いため、`L`の右隣の`;`に、`-`を割り当てています。  

  ホームポジションから外れる`[`,`]`,`;`,`'`などの記号キーは手の移動が面倒なので、物理的な配置はおこなわずにレイヤー内に配置しました。  
  また、1行目の数字行を打鍵するには腕の移動が必要となるため、レイヤー内の2行目に配置した数字と記号をホームポジションのまま打鍵するが多くなってきました。  
  少しずつ40%キーボードに近づいている感じですが、1行目の数字行の文字を見ながらレイヤー内の2行目を打鍵しているので、まだしばらくはこのままでしょう。

- レイヤー
  7個のレイヤーを使用しています。  
  工夫すればもう少し減らせそうなのですが、外部EEPROMで使えるレイヤー数に余裕があったので、こうなっています。
  ...
  | 5 | Temporal Alpha | Numeric（テンキー）使用時に、ちょこっとだけ文字入力するだけのレイヤー |
...
`Raise`レイヤーでカーソル移動できるキーマップなので、カーソルキーは省いています。

私は肘から少し先までを机に乗せて、手をあまり浮かせずに打鍵するタイプなので、小指の付け根付近が最下行のキーに当たりやすく、誤打鍵はしないものの少し不快に感じます。  
...
左下のロータリエンコーダは、動画の視聴やラジオを聴取しているときに、ささっとボリュームを変えるのに使っています。
...
基本的には自宅で使用しているものと同じですが、独自ソフトでカーソルキーの使用頻度が高いのと、右手にペンを持ったままカーソル操作を強いられることが多いため、カーソルキーを単独で配置している点が自宅との大きな違いです。  
また、Windows PCのため、Windows向けのModキーに設定したベースレイヤーに切り替えています。

数値データの大量入力は専用のNumericレイヤーでおこなっており、Realforceのテンキーで入力していた頃と変わらない速度で入力できています。  
...
特にピンク系のキーキャップは、さらにたくさん声をかけられます。
