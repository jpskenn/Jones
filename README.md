...
Jonesキーボードは、次のようなユーザーにおすすめです。

Jonesキーボードは、一般的なキーボードのキー配列に不満があり、より良いタイピングポジションを求めている方を対象に作られています。

具体的には次のような方におすすめです。

- 一般的なキーボードのキー配列に不満がある
- 右手と左手のポジションを改善したい
- アルファ部やModを左右対称にしたい
- 一体型のキーボードを使いたい
- 左手でYを打鍵したり、右手でBを打鍵する
- オルソリニアやカラムスタガは手に合わない
- 40%や50%キーボードは、キー数が少なくて不安
- 数字や記号をレイヤーに入れたくない（物理的に必要）
- 数字行が必要
- 60%キーボードのサイズ感が好み
- スプリットスペースを使いたい
- 親指を活用できるキー配列を求めている
- ケースを着せ替えしたい
- ファームウェアがよくわからないので、キーマップ書き換えツールを使いたい
- 組み立てが簡単な自キを探している
- 一般的なキー配列から大きく変えるのは不安

keycap NP PBT \10k
keycap FILCO \3k
kit (kbd, sw etc., keys) \26-28k
