...
- キーマップ  
  英語配列を基本としたキーマップです。  
  日本語で長音（伸ばし棒）の`ー`を入力することが多いため、`L`の右隣の`;`に、`-`を割り当てています。  
...
- レイヤー  
  7個のレイヤーを使用しています。  
  工夫すればもう少し減らせそうなのですが、外部EEPROMの搭載で使えるレイヤー数に余裕があるので、工夫するのは先延ばししています。
...
- レイヤーインジケータLED  
  ...
  左Ctrlを1.25uのサイズにして、インジケータLEDが見やすいようにしています。
...
`Raise`レイヤーでカーソル移動ができれば十分なので、カーソルキーは省いています。

左手でマウスを使用するので、マウス主体のアプリケーションを使用する際に右手でModキーが操作できるよう、右手側のModキー（Cmd, Opt, Shift, Ctrl）をそれぞれ単独で配置しています。
...
ロータリーエンコーダも、軸が短いタイプに背の低いノブを組み合わせて、手のひらに当たらないようにしています。
...
数値データの大量入力は専用のNumericレイヤーに切り替えておこなっています。  
テンキー部分を右手のホームポジションに配置してあるので、数値入力中にBackspaceで削除したり、Lowerレイヤーでカーソル移動したりする操作を右手を移動させずにおこなうことができ、普通のテンキーに比べて作業効率が上がっています。

共用PCで作業することが日に何度もあり、そのときは一般的な日本語108キーボードを使用しています。  
右手小指で打鍵するEnterがとても遠くに感じますが、Jonesのレイアウトが特殊すぎない範囲に収まっているので、違和感を感じることもなく、誤打鍵せずに使えています。

余談ですが、緑色や水色などの色付きキーキャップを使用すると、キーボードについて声をかけられる率が上がります。  
特にピンク系で明るい色のキーキャップは、さらにたくさん声をかけられます。
