...
40%や50%に分類されるキーボードが60%のケースに収まったようなレイアウトになっており、コンパクトで軽快な運指の心地良さと、レイヤーに記号キーを押し込む必要のない十分なキー数を兼ね備えています。

PCBとキープレートはGH60型・Poker互換のケースと組み合わせて使用できます。

〓Remap、オプションはここに書かない〓
RemapやVIAといったキーマップの書き換えツールに対応しており、増量されたEEPROMによって多数のレイヤーを使用することができます。  

またオプションとして、レイヤーインジケータとして使用できるLEDや、各種キーマクロを割り当てられる左右2個のロータリーエンコーダー、そして、起動音やキーのクリック音を再生できるスピーカーを使用することができます。
...
40%キーボードのコンパクトなレイアウトにならい、Modキーを右手のホームポジションのすぐ隣になるように配置しました。  
...
また、Wideレイアウトのキーマップを調整したNormal（ノーマル）レイアウトでは、一般的な英語配列や日本語配列として使うこともできます。  

- Wide（ワイド）レイアウト
  右手ホームポジションをModキーに近づけ、キーボード中央に2キー分の間隔をもたせたレイアウトです。  
  キーボード全体を左右対称にすることができます。  
  左右のホームポジションが少し離れるため、肩への負担を軽減できます。
  〓写真Wide〓

- Narrow（ナロー）レイアウト
  Modキーを右手ホームポジションに近づけたレイアウトです。  
  40%キーボードの上に数字行を追加し、右に2列増やしたようなレイアウトになっています。  
  アルファ部とModを左右対称にしつつ、右端2列の非シンメトリカルな存在を楽しむことができます。
  〓写真、Narrow、右2列を記号ではなく、移動やノベルティーにしたもの〓

- Normal（ノーマル）レイアウト
  Wideレイアウトのキーマップを調整した、一般的な英語配列や日本語配列のキーボードとほぼ同じレイアウトです。  
