...
私がどのようにJonesキーボードを使っているのかを共有することで、このキーボードの何が良いのか、どういうシーンで使っているのかを伝えたいと思います。
...
細かな変更はありますが、2行目と3行目にずれのないレイアウトを、自宅と会社のメインキーボードとして1年以上（2021年9月現在）使い続けています。

〓使いにくいものだったら、こんなには使い続けないでしょうから、
〓Realforceｍ所有しているが、少なくとも、それよりは使いやすいのでしょう。

英語配列を基本として、試作の初期段階から1年近くはNarrowレイアウトで使ってきましたが、現在（2021年9月）は親指を活用できるWideレイアウトで使っています。

私がどのようにJonesキーボードを使っているのかを共有することで、このキーボードがどういうものなのかを伝えたいと思います。
...
右手親指のスペースキーを押し続けると`Raise`レイヤーになり、  
`h`, `j`, `k`, `l`でカーソル`←`, `↓`, `↑`, `→`の小移動、  
1行下がった`n`, `m`, `,`, `.`が`Home`, `PgDown`, `PgUp`, `End`の大移動
というキーマップなので、カーソルキーは省いています。
