...
- 一部の金属製ケースと組み合わせて使用した場合、誤入力が発生する場合があります。

    [誤入力対策](#誤入力対策)を参照してください。

- v.0.3の基板設計のミスにより、次の制限があります。  
    - [JPスタイルのレイアウト](http://www.keyboard-layout-editor.com/#/gists/3bc20c5810c8dc7985cc79059cf4c005)のみに対応します
    - スピーカーのサポートなし
...
Hを押下したとき、カンマ（,）とドット（.）も同時に入力される問題への対策をおこないます。

以下の対策方法A、または対策方法Bのいずれかの対策をおこないます。
##### 対策方法A
基板おもて側で、Hキーの場所を確認します。

![](../assets/BuildGuide_v.0.3/IMG_2354.jpeg)
Hキーの場所

ネジ穴をぐるっと迂回する配線から、ネジ穴に向かって飛び出した2〜3mmの配線をカットします。 
カッターやマイナスドライバーを使い、他の配線を傷つけないように注意しながら、少しずつ削ります。 

1回でカットしようとせず、数回〜20回くらいにわけて少しずつ削ります。

![](../assets/BuildGuide_v.0.3/IMG_2355.jpeg)
カット箇所　赤線の箇所をカットする

配線をカットしたら、基板をうら側にします。 
ネジ穴とダイオードD78の下側を、テスターで導通が**ない**ことを確認します。
![](../assets/BuildGuide_v.0.3/IMG_2358.jpeg)

##### 対策方法B

中央のネジ穴をセロテープなどで絶縁します。 
ネジやケースに触れる、おもて、うら、穴の内側にテープを貼ってください。

![](../assets/BuildGuide_v.0.3/IMG_2356.jpeg)
ネジ穴の絶縁例, 基板おもて側（写真は下のネジ。わかりやすいようマスキングテープを使用）

![](../assets/BuildGuide_v.0.3/IMG_2357.jpeg)
ネジ穴の絶縁例, 基板うら側（写真は下のネジ。わかりやすいようマスキングテープを使用）


#### カンマ（,）

カンマ（,）を押下したとき、Hとドット（.）も同時に入力される問題への対策をおこないます。

以下の対策方法C、または対策方法Dのいずれかの対策をおこないます。

##### 対策方法C

下のネジを止めず、基板うら側からネジ穴部分をセロテープなどで絶縁します。

##### 対策方法D

下または右上のネジ穴のどちらかを、セロテープなどで絶縁します。 
ネジやケースに触れる、おもて、うら、穴の内側にテープを貼ってください。
...
- QMKコマンド用キーボード関連ファイル：  
[jpskenn/qmk_firmware: develop_Jonesブランチ](https://github.com/jpskenn/qmk_firmware/tree/develop_Jones/)
...
[Jones v.0.3 KiCadプロジェクト](https://github.com/jpskenn/Jones/tree/PCB_v.0.3/PCB)のPCB設計を参照し、パッド、ダイオード、MCUのピンなどの導通を確認して調査します。  
