...
〓テープLEDは参考に書いて、リンクつける。ファーム書き換え必要なので
...
正常に入力できないスイッチがあった場合は、[参考：正常に入力できないスイッチの原因調査](#参考：正常に入力できないスイッチの原因調査)の手順に沿って調査します。
...
[QMK環境の構築](https://docs.qmk.fm/#/ja/newbs_getting_started)を参考にビルド環境を構築します。

必要に応じてキーマップや機能の設定を変更します。

次のようにファームウェアを構築します。  
`via`キーマップでファームウェアを構築する例

``` shell
qmk compile  -kb jones/v04_1 -km via
```

[ファームウェアを書き込む](https://docs.qmk.fm/#/ja/newbs_flashing)を参考に、構築したファームウェアを書き込みます。
