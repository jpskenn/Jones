...
LEDの視認性を向上するため、キープレートのLED窓部分に、プラ板をディフューザーとして取り付けます。
...
〓ケースにつけるときに外さないといけないので、準備するだけでつけない
...
右下は、WideレイアウトとNarrow, Normalレイアウトで位置が異なります。
...
RemapやVIAなどのキーマップ書き換えツールで設定した内容を消去し、ファームウェアで指定された初期状態に戻すことができます。  
なお、設定した内容を消去すると、LEDやスピーカーなどの設定も初期状態に戻ります。
