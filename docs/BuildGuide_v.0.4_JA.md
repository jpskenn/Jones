...
ロータリーエンコーダのピン、上3本, 下2本（プッシュボタン付きの場合のみ）と、左右の固定足を、基板の設置位置に合わせて差し込みます。  
...
以下の写真に示す〓数〓箇所をねじ止めします。
〓写真：スペーサーねじ止め箇所

ボトムプレートのスペーサーの位置に合うように、基板をそっと乗せます。
...
- キーボードをフラットにする場合
    ゴム足 小：手前と奥の左・中・右の6箇所

### （該当者のみ）60% プラスチックケースの加工

*60% プラスチックケースを使用しない場合、この手順は不要です。*

60% プラスチックケースへ取り付ける際は、ケース内リブが基板裏側の部品と干渉するため、干渉箇所を切り取る加工を行います。
...
通常、ネジ左右4本、中央1、中央下1
ネジ左右4本だけ、ネジ穴カットなし
基板のねじ穴を一部カット、左右4本ねじ止め
基板のねじ穴を全部カット、スポンジマウント
...
1. 光らないLEDのはんだ不良を修復する
    ...
    光らないLEDの側面の端子部分にフラックスを塗り、こて先につけたはんだを押し当てて、はんだ付けをやり直します。
