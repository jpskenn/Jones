...
このドキュメントは、Jones v.0.4を組み立てるためのビルドガイドです。 
Jonesのコンセプトや特徴などについては、[Jones README](https://github.com/jpskenn/Jones/blob/master/README.md)をご覧ください。

Jones v.0.4は、主要な部品がはんだ付けされ、ファームウェアを書き込んだ状態で出荷しています。 
基本的な組み立て作業の難易度はそれほど高くありませんが、ビルドガイド全体に目を通して作業手順を把握しておくとミスの防止につながりますので、ぜひ、一度お読みになってから組み立て作業を始めてください。
...
5pinスイッチの片方の足を切断。（3pinスイッチは影響なし）
  ...
  - 右上ネジ穴：2行目の右端の1.25uのキー
5pinスイッチの片方の足を切断。（3pinスイッチは影響なし）

  - 右上ネジ穴：Narrowレイアウトの2行目の右から2個目のキー
スイッチ中央の足（出っ張り）を切断。

- キースイッチの取り付け方向
一部のキーは上下逆向きや横向きに取り付けるため、キーキャップが取り付けにくかったり、キーキャップとスイッチが干渉し打鍵感が悪くなることがあります。
場合によってはキーキャップやスイッチが破損するかもしれません。
（現在のところ、破損の報告はありません）
...
- ケース
GH60型に対応する基板とキープレートですが、もしかしたら、取り付けできないケースがあるかもしれません。
取り付け確認済みのケースは次の通りです。  
...
| 部品名 | 個数 | 備考 |
| ----- | -----: | ----- |
| MXスイッチ | 最大 70 | 取り付けの安定性が高い5pinのものを推奨 |
...
なお、無くても組み立てはできますが、何か不具合があった場合の原因究明のために、安いものでもかまわないのでテスターがあると便利です。
...
〓写真更新〓![基板おもて側](../assets/BuildGuide_v.0.3.1/DSC_6972.jpeg)
...
キースイッチをはめ込むための、穴がたくさんあいた枠です。

- おもて側  
右上に長方形の枠がある方が、おもて側です。
〓写真更新〓![](../assets/BuildGuide_v.0.3.1/DSC_6968.jpeg)

- 裏側
Jonesのロゴが描かれている方が、裏側です。
〓写真更新〓
...
## （使用する場合のみ）Jones用ボトムプレートキットの概要

### Jones用ボトムプレート

- おもて側（基板側）
Jonesのロゴが描かれている方が、おもて側です。
〓写真更新〓
...
3、ソケットを動かしてしまわないよう、しずかにこて先を離します。
