...
キーレイアウトを見ながら全ての組み合わせが確認できるので、Remapを使って確認するのがおすすめです。

他にも、OSのキーボードビューアや、[Keyboard Checker](https://keyboardchecker.com)、[QMK Configrator](https://config.qmk.fm/)のキーボード入力テストを使用して確認することもできます。

#### Remapを使った確認方法

基板の裏面が上側になるように置き、PCやMacとUSBケーブルで接続します。

Google Chromeブラウザ（バージョン89以降のWeb HIDに対応したもの）で、[Remap](https://remap-keys.app/)を開きます。

`START REMAP FOR YOUR KEYBOARD`をクリックしてキーボード接続画面に進みます。

`+ KEYBOARD`をクリックし、ポップアップ画面を開き、  
`Jones`を選択して`接続`をクリックすると、画面にレイアウトが表示されます。
...
また、レイアウト右側の`Layout Option`アイコンをクリックし、他のレイアウトでも同様に確認します。
...
聞き手にはんだごて、もう一方の手にはんだを持ちます。

パッドにこて先をあてて、1、2、と数えて加熱します。
...
残りの端子の隙間にこて先を押し込み、1、2　　と数えて加熱します。
...
基板裏側のジャンパ`Right LEDs`と`Left LEDs`を、インジケータLEDの組み合わせに応じて`On`または`Off`にジャンパ（パッドにはんだをつけてつなげる）する手順です。  
...
〓Remapなテスト。テスト前にレイアウト合わせてから〓

正常に入力できないスイッチがあった場合は、〓リンク〓参考：正常に入力できないスイッチの原因調査方法の手順に沿って調査します。
...
デフォルトのキーマップでは、左下、右下ともに、同じ機能が割りあてられています。
