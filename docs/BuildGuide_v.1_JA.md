...
ファームウェアが書き込み済みなので、基本的な組み立ては、MXソケットをはんだ付けしてケースに組み合わせれば完了します。
...
- [参考](#参考)
  ...
  - [参考：初期ファームウェアの機能と設定値](#参考初期ファームウェアの機能と設定値)
  - [参考：Jonesキーボードのファームウェア](#参考jonesキーボードのファームウェア)
...
- [参考：1個のスピーカーで2音同時再生する方法](#参考1個のスピーカーで2音同時再生する方法)
...
[参考：ファームウェアの書き込み方法](#参考ファームウェアの書き込み方法)を参考に、ファームウェアを書き込みます。  
...
この操作をおこなった場合は、ファームウェアの書き込み後に[参考：EEPROMを消去して、初期状態に戻す方法](#参考eepromを消去して-初期状態に戻す方法)を実施して、EEPROMを初期状態に戻す必要があります。
